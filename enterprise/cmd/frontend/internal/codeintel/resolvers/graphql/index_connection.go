...
func NewIndexConnectionResolver(resolver resolvers.Resolver, indexesResolver *resolvers.IndexesResolver, locationResolver *CachedLocationResolver) gql.LSIFIndexConnectionResolver {
...
func (r *IndexConnectionResolver) Nodes(ctx context.Context) ([]gql.LSIFIndexResolver, error) {
	...
	for i := range r.indexesResolver.Indexes {
		resolvers = append(resolvers, NewIndexResolver(r.indexesResolver.Indexes[i], r.resolver, r.locationResolver))
	}
